...
public class Blurl {
	...
	public static void main(String[] args) throws DataFormatException, IOException {
		File file = new File("D:\\Downloads\\master (2).blurl");
		if (!file.exists() || !FilesKt.getExtension(file).toLowerCase(Locale.ROOT).equals("blurl")) {
			throw new IOException("File does not exist or is not a blurl file");
		}
		byte[] b = FilesKt.readBytes(file);
		JsonObject blurl = decompress(b);
	...
	public static JsonObject decompress(byte[] b) throws DataFormatException, IOException {
