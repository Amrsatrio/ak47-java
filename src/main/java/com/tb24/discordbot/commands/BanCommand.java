...
public class BanCommand {
	...
	public static void register(CommandDispatcher<CommandSourceStack> var0) {
		var0.register(LiteralArgumentBuilder.<CommandSourceStack>literal("ban")
			.requires(source -> source.isFromType(ChannelType.TEXT)
				&& source.getMember().hasPermission(Permission.KICK_MEMBERS)
				&& source.getGuild().getSelfMember().hasPermission(Permission.KICK_MEMBERS))
			.then(RequiredArgumentBuilder.<CommandSourceStack, MentionArgument.Resolver>argument("targets", MentionArgument.mention(Message.MentionType.USER))
				.executes(context -> execute(context.getSource(), MentionArgument.getMention(context, "targets"), "You have been banned from the server."))
				.then(RequiredArgumentBuilder.<CommandSourceStack, String>argument("reason", StringArgumentType.greedyString())
					.executes(context -> execute(context.getSource(), MentionArgument.getMention(context, "targets"), StringArgumentType.getString(context, "reason")))
				)
			)
		);
	}
	...
	private static int execute(CommandSourceStack source, Collection<IMentionable> targets, String reason) throws CommandSyntaxException {
		...
		for (IMentionable target : targets) {
			...
			try {
				source.getGuild().getMember(user).ban(0, reason).complete();
