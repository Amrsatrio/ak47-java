...
private fun dumpClass(source: CommandSourceStack, path: String, functionBodyOption: EFunctionBodyOption = EFunctionBodyOption.NoBody): Int {
	...
	val sw = StringWriter()
	sw.append("// Class dump generated by ").append(source.message.jda.selfUser.asTag).append('\n')
	sw.append("// Requested by ").append(source.author.asTag).append(when {
		source.isFromType(ChannelType.PRIVATE) -> " in DMs"
		source.isFromType(ChannelType.TEXT) -> " at %s".format(source.guild.name)
		else -> ""
	}).append("\n\n")
