...
public fun FlowColumn(
    ...
    crossAxisSpacing: Dp = 0.dp,
    lastLineMainAxisAlignment: FlowMainAxisAlignment = mainAxisAlignment,
    mainAxisTarget: Dp = Dp.Unspecified, // MOD,
    content: @Composable () -> Unit
) {
    Flow(
        modifier = modifier,
...
private fun Flow(
    ...
    Layout(content, modifier) { measurables, outerConstraints ->
        ...
        val constraints = if (mainAxisTarget != Dp.Unspecified) {
            OrientationIndependentConstraints(0, Int.MAX_VALUE, 0, Int.MAX_VALUE)
        } else {
            OrientationIndependentConstraints(outerConstraints, orientation)
        }
        ...
        val actualMainAxisMax = if (mainAxisTarget != Dp.Unspecified) {
            mainAxisTarget.roundToPx()
        } else {
        ...
        fun canAddToCurrentSequence(placeable: Placeable) =
            currentSequence.isEmpty() || currentMainAxisSize + mainAxisSpacing.roundToPx() +
                placeable.mainAxisSize() <= actualMainAxisMax
        ...
        val mainAxisLayoutSize = if (actualMainAxisMax != Constraints.Infinity &&
